...
public:
    ...
    // skiplist object.
    explicit InlineSkipList(Comparator cmp,
                            Allocator *allocator,
    ...
    else
    {
        ...
        while (recompute_height < max_height)
        {
                ...
                splice->next_[recompute_height])
            {
                ...
                // inserts
                // to this
                // location that didn't update the splice.  We might only be a
                // little
                // stale, but if
                // the splice is very stale it would be O(N) to fix it.  We
                // haven't used
                // up any of
