...
for (( i=1; i<${#vms[@]}; i++ )); do
    ...
    nohup ssh root@${inets[$i]} "cd ${BIN_DIR}; source /etc/profile; export TERM='linux'; unbuffer ${BIN_DIR}/$cmd $@ ${GLOG_FLAGS} 1>${WORK_DIR}/LOG 2>&1" &
done

./ssh.sh ${inets[0]} "cd ${BIN_DIR}; source /etc/profile; export TERM='linux'; unbuffer ${BIN_DIR}/$cmd ${GLOG_FLAGS} $@ 2>&1 | tee ../LOG"
