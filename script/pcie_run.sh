...
./ssh.sh ${inets[0]} "cd ${BIN_DIR}; source /etc/profile; export TERM='linux'; pcm-pcie 0.1 -e -B -silent -csv=pcie.${cmd}.${exec_meta}.csv -- ${BIN_DIR}/$cmd ${EXEC_META_FLAGS} $param 2>&1 | tee ../LOG"
