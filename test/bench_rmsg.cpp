            ...
            [dsm, i, &continue_token]()
            {
                ...
                for (size_t t = 0; t < kBurnCnt; ++t)
                {
                    ...
                    if (sent % msg_each_token == 0)
                    {
                        ...
                        do
                        {
                            size_t recv_nr = dsm->reliable_try_recv(buffer, 64);
                        ...
                        DVLOG(3) << "[wait] tid " << tid << " from mid " << mid
                 ...
                 size_t thread_nr)
{
    ...
    using Aligned =
        std::aligned_storage<sizeof(std::atomic<int64_t>), 128>::type;
    Aligned recv_mid_msgs_layout[RMSG_MULTIPLEXING];

    for (size_t i = 0; i < thread_nr; ++i)
    {
        threads.emplace_back(
            [dsm, i, &got, total_msg_nr, &recv_mid_msgs_layout]()
            {
                bindCore(i + 1);
                dsm->registerThread();
                auto tid = dsm->get_thread_id();
                ...
                // while (true)
                {
                    ...
                    for (size_t i = 0; i < get; ++i)
                    {
                        ...
                        std::atomic<int64_t> &atomic_cnt =
                            *(std::atomic<int64_t>
                                  *) &recv_mid_msgs_layout[recv_msg->mid];
                        auto now = atomic_cnt.fetch_add(1) + 1;
                        ...
                        {
                            atomic_cnt.fetch_sub(credit_for_token);
