...
void test_basic(Patronus::pointer patronus, size_t initial_subtable)
{
    auto [rh_ptr, rhh_ptrs] = gen_rdma_rh<4, 64, 64>(
        patronus, initial_subtable, 1 /* thread_nr */, false /* auto-expand */);
    auto &rh = *rh_ptr;
    auto &rhh = *rhh_ptrs[0];
    ...
    tear_down_rdma_rh<4, 64, 64>(rh_ptr, rhh_ptrs);
}

void test_capacity(Patronus::pointer patronus, size_t initial_subtable)
{
    auto [rh_ptr, rhh_ptrs] = gen_rdma_rh<4, 16, 16>(
        patronus, initial_subtable, 1 /* thread_nr */, false /* auto-expand */);
    auto &rh = *rh_ptr;
    auto &rhh = *rhh_ptrs[0];
    ...
    tear_down_rdma_rh<4, 16, 16>(rh_ptr, rhh_ptrs);
}
...
void test_multithreads(Patronus::pointer patronus,
                       size_t thread_nr,
                       size_t test_nr,
                       bool expand)
{
    auto [rh_ptr, rhh_ptrs] = gen_rdma_rh<kDEntryNr, kBucketGroupNr, kSlotNr>(
        patronus, 1, thread_nr, expand);
    ...
    tear_down_rdma_rh<kDEntryNr, kBucketGroupNr, kSlotNr>(rh_ptr, rhh_ptrs);
}

void test_expand_once_single_thread(Patronus::pointer patronus)
{
    auto [rh_ptr, rhhs_ptr] = gen_rdma_rh<2, 4, 4>(patronus, 1, 1, false);
    auto &rh = *rh_ptr;
    auto &rhh = *rhhs_ptr[0];
    ...
    tear_down_rdma_rh<2, 4, 4>(rh_ptr, rhhs_ptr);
}

void test_expand_multiple_single_thread(Patronus::pointer patronus)
{
    auto [rh_ptr, rhh_ptrs] = gen_rdma_rh<16, 4, 4>(patronus, 1, 1, true);
    auto &rh = *rh_ptr;
    auto &rhh = *rhh_ptrs[0];
    ...
    tear_down_rdma_rh<16, 4, 4>(rh_ptr, rhh_ptrs);
}

void test_burn_expand_single_thread(Patronus::pointer patronus)
{
    auto [rh_ptr, rhh_ptrs] = gen_rdma_rh<128, 2, 2>(patronus, 1, 1, true);
    auto &rh = *rh_ptr;
    auto &rhh = *rhh_ptrs[0];
    ...
    tear_down_rdma_rh<128, 2, 2>(rh_ptr, rhh_ptrs);
}
...
int main(int argc, char *argv[])
{
    ...
    CHECK(false) << "let server be node_id == 1, so that the gaddr.nodeID is "
                    "not zero. see if its correct";

    CHECK(false) << "patronus init";
    PatronusConfig pconfig;
    pconfig.machine_nr = kMachineNr;

    auto patronus = Patronus::ins(pconfig);

    CHECK(false) << "handle client and server code path";
    test_basic(patronus, 1);
    test_capacity(patronus, 1);
    test_capacity(patronus, 4);

    test_multithreads<4, 8, 8>(patronus, 8, 100_K, false);

    test_expand_once_single_thread(patronus);

    test_expand_multiple_single_thread(patronus);
    test_burn_expand_single_thread(patronus);
