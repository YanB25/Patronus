...
void send_cas(int node_id, int thread_id) {
  ...
  while (true) {
    ...
    // sleep
    // dsm->write((char *)buffer, gaddr, sizeof(uint64_t), (sendCounter & SIGNAL_BATCH) == 0);
    dsm->cas_dm(gaddr, new_val - 1, new_val, buffer + kth,
...
void send_skew_cas(int node_id, int thread_id) {
  ...
  while (true) {
    ...
    dsm->cas(gaddr, 0, 100, buffer + gaddr.offset,
             (sendCounter & SIGNAL_BATCH) == 0);
