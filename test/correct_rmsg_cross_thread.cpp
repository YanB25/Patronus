...
constexpr static size_t kBurnCnt = 400 * define::K;
...
void server_burn(DSM::pointer dsm,
...
void client_burn(DSM::pointer dsm, size_t tid, size_t offset)
{
    auto from_mid = (tid + offset) % RMSG_MULTIPLEXING;
    CHECK_NE(from_mid, 0);
    ...
    for (size_t time = 0; time < kBurnCnt; ++time)
    {
        ...
        if (sent % 64 == 0)
        {
            DVLOG(3) << "[wait] tid " << tid << " sent 64. wait for ack. ";
            dsm->reliable_recv(from_mid, buffer);
            auto *recv_msg = (BenchMessage *) buffer;
            DVLOG(3) << "[wait] tid " << tid << " recv continue msg for mid "
                     << recv_msg->from_mid;
        }
