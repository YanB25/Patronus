...
//     CHECK_EQ((uint64_t) recv_msg_pool_ % 64, 0) << "must be cacheline
//     aligned.";
...
//     send_mr_ = CHECK_NOTNULL(createMemoryRegion((uint64_t) mm, mm_size,
//     &ctx_)); send_lkey_ = send_mr_->lkey;
...
//                                 ctx_.res_doms[i] ? ctx_.res_doms[i] :
//                                 nullptr));
//     }
//     // at maximum, the sender will have (RMSG_MULTIPLEXING) *
//     kSenderBatchSize
//     // pending cqes
...
//     size_t qp_max_depth_send = kSenderBatchSize * kMaxAppThread;
...
//     size_t threadID, const char *buf, size_t size, uint16_t node_id, size_t
//     mid)
// {
