...
void bench_mw_launcher(DSM::pointer dsm,
                       size_t bind_size,
                       size_t test_times,
                       size_t thread_nr)
{
    ...
    std::vector<std::thread> threads;
    std::vector<std::vector<ibv_mw *>> mw_pool_threads;
    mw_pool_threads.resize(thread_nr);
    for (size_t i = 0; i < thread_nr; ++i)
    {
        constexpr static ssize_t kAllocMwNr = 64;
        static_assert(kAllocMwNr >= 4 * kBatchSize,
                      "Recommend turn up kAllocMwNr.");
        for (size_t t = 0; t < kAllocMwNr; ++t)
        {
            size_t dir_id = i % NR_DIRECTORY;
            // NOTE: this dir_id should match the one below
            mw_pool_threads[i].push_back(dsm->alloc_mw(dir_id));
        }
    }

    // launcher (n-1) threads
    ChronoTimer timer;
    for (size_t i = 0; i < thread_nr; ++i)
    {
    ...
    // run one thread
    // bench_mw_do(dsm, 0, test_times, bind_size, mw_pool_threads[0],
    // kBatchSize);
    ...
    LOG(INFO) << "[bench] bind MW with threads " << thread_nr << ", "
              << util::pre_ops(total_test_times, ns, true)
              << ", per_thread: " << util::pre_ops(test_times, ns, true);

    // std::vector<std::vector<ibv_mw *>> mw_pool_threads;
    for (const auto &arr : mw_pool_threads)
    {
        for (auto *mw : arr)
        {
            dsm->free_mw(mw);
...
void bench_mw()
{
    ...
    {
        bench_mw_launcher(dsm, 64, 2_M, thread_nr);
    }
}
